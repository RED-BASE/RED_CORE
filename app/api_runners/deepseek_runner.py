...
try:
    ...
    raise ImportError("config.py with DEEPSEEK_API_KEY must be present in project root.")

class DeepSeekRunner(BaseAPIRunner):
    """
    Wrapper around DeepSeek API for RED_CORE integration.
    Returns structured outputs for logging + schema integration.
    """

    def __init__(self, api_key: str | None = None, model_name: str = "deepseek-chat"):
    ...
    def _count_tokens(self, text: str) -> int:
        ...
        Uses 4 characters per token as rough estimate since DeepSeek doesn't provide tokenizer.
        """
        return len(text) // 4
    ...
    def _make_api_call(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        if conversation is not None:
            messages = conversation.to_openai_format()  # DeepSeek uses OpenAI-compatible format
        else:
            messages = []
        ...
        response = requests.post(
            ...
            headers=headers,
            json=payload,
            timeout=120  # DeepSeek can be slower for code generation
        )
    ...
    def generate(
        ...
        **kwds,
        ...
        dict
            ...
            ``None``) and ``raw_response`` (the raw DeepSeek response object).
        """
    ...
    def _build_kwargs(kwds: Dict[str, Any]) -> Dict[str, Any]:
        allowed = {
            "temperature": 1.0,
            "max_tokens": 2048,  # DeepSeek supports longer responses
            "top_p": 0.95,
            "stream": False,
...
if __name__ == "__main__":
    print("DeepSeekRunner smoke-test OK")