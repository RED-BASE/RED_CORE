...
try:
    ...
    raise ImportError("config.py with XAI_API_KEY must be present in project root.")

class XAIRunner(BaseAPIRunner):
    """
    Wrapper around XAI (Grok) API for RED_CORE integration.
    Returns structured outputs for logging + schema integration.
    """

    def __init__(self, api_key: str | None = None, model_name: str = "grok-beta"):
        ...
        super().__init__(provider_name="xai", api_key=api_key)
    ...
    def _count_tokens(self, text: str) -> int:
        """
        Estimate token count for XAI Grok models.
        Uses 4 characters per token as rough estimate since XAI doesn't provide tokenizer.
        """
        return len(text) // 4

    def _make_api_call(self, prompt: str, **kwargs) -> Dict[str, Any]:
        """
        Make the actual XAI API call.
        ...
        if conversation is not None:
            messages = conversation.to_openai_format()  # XAI uses OpenAI-compatible format
        else:
            messages = []
    ...
    def generate(
        ...
        **kwds,
        ...
        """Generate a completion from XAI Grok.
        ...
        dict
            ...
            ``None``) and ``raw_response`` (the raw XAI response object).
        """
